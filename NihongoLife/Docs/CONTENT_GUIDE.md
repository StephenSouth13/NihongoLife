...
**Chưa thanh toán mà ra khỏi quán**: từ lúc gọi món tới lúc trả tiền, bàn giữ một hóa đơn chưa thanh toán. (1) Cửa ra (`ScenePortal` có `exitsZone`) bị chặn qua `ExitGuard`/`IExitBlocker` và nhân viên nhắc bằng phụ đề (khoá `exit_blocked`, có ghi chú 食い逃げ là vi phạm pháp luật). (2) Trên HUD, giữa phía trên màn hình, có chip thông báo vàng dấu "!" hiện số tiền và trạng thái hóa đơn, nháy khi đang chờ thanh toán; bấm chip (khi con trỏ đang tự do) để nghe nhắc lại. (3) Sau khi ăn xong 25 giây chưa trả, nhân viên nhắc `bill_reminder`, lặp mỗi 30 giây. Cơ chế chặn dùng chung: quán/cửa hàng khác chỉ cần implement `IExitBlocker`.
